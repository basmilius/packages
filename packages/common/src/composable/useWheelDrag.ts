import { type MaybeRef, onScopeDispose, readonly, ref, type Ref } from 'vue';
 ...
 */
export default function <TElement extends HTMLElement>(elementRef: MaybeRef<TElement | null | undefined>, options: UseWheelDragOptions = {}): UseWheelDragReturn {
    ...
    function wheelScale(deltaMode: number): number {
        switch (deltaMode) {
            ...
            case WheelEvent.DOM_DELTA_PAGE:
                return (axis === 'x' ? unwrapElement(elementRef)?.clientWidth : unwrapElement(elementRef)?.clientHeight) ?? 0;
            default:
                return 1;
        }
    }
